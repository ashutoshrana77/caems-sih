    ...
    <div className="site-shell">
      <header className="topbar">
        ...
        <nav className={menuOpen ? "nav-links open" : "nav-links"}>
          ...
          <button className="nav-cta" onClick={() => runAction("Pilot briefing")}>Request pilot <ArrowUpRight size={15} /></button>
        </nav>
      ...
      <main id="top">
        ...
        <section className="dashboard-section dossier-light">
          ...
          {activeMode === "citizen" ? <div className="phone-evidence"><div className="evidence-note note-left"><span>DEVICE / CITIZEN 01</span><b>Offline ready</b><small>Last map sync<br />08:39 · Ward 08</small></div><div className="phone-demo"><div className="phone-notch" /><div className="phone-top"><span>CAEMS</span><Signal size={14} /></div><div className="phone-alert"><div className="alert-top"><span className="live-dot" /> LIVE ALERT <small>08:42</small></div><h3>Heavy rainfall advisory</h3><p>Ward 08–12 · Moderate flood risk predicted in next 3 hours.</p><button onClick={() => runAction("Safe route")}>View safe route <ArrowUpRight size={14} /></button></div><div className="phone-section-title">YOUR READINESS</div><div className="phone-check"><Check /> <span>Offline map downloaded</span><b>DONE</b></div><div className="phone-check"><Check /> <span>Household checklist</span><b>6 / 8</b></div><button className={sosHeld ? "sos-button holding" : "sos-button"} onPointerDown={beginSOS} onPointerUp={cancelSOS} onPointerLeave={cancelSOS} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") beginSOS(); }} onKeyUp={(event) => { if (event.key === "Enter" || event.key === " ") cancelSOS(); }} aria-label="Hold to send SOS"> <Crosshair size={17} /> {sosHeld ? "KEEP HOLDING…" : "HOLD FOR SOS"}</button></div></div> : <div className="authority-demo"><div className="dash-top"><span><span className="live-dot" /> DISTRICT COMMAND / LIVE</span><small>Last sync 08:42:18</small></div><div className="dash-grid"><div className="dash-map"><div className="map-grid-lines" /><span className="dash-route" /><span className="dash-pin dp1" /><span className="dash-pin dp2" /><span className="dash-pin dp3" /><div className="map-legend"><span><i className="orange-bg" /> active incident</span><span><i className="teal-bg" /> safe route</span></div></div><div className="dash-side"><div className="dash-stat"><small>OPEN SOS</small><strong>18</strong><span className="up">+4 today</span></div><div className="dash-stat"><small>VOLUNTEERS ACTIVE</small><strong>64</strong><span>of 82 registered</span></div><div className="dash-stat"><small>RELIEF INVENTORY</small><strong>76%</strong><span className="warn">2 items low</span></div></div></div></div>}
        </section>
